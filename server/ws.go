...
func (s *Server) AcceptWSLoop() {
	...
	http.Handle(s.info.WsPath, websocket.Handler(s.HandleWS))
	log.Info("\tListen websocket on port: ", s.info.WsPort)
	if s.info.WsTLS {
		http.ListenAndServeTLS(":"+s.info.WsPort, s.info.TlsInfo.CertFile, s.info.TlsInfo.KeyFile, nil)
		// http.ListenAndServeTLS(addr, certFile, keyFile, handler)
	} else {
		http.ListenAndServe(":"+s.info.WsPort, nil)
	}
